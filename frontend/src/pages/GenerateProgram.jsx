  ...
  useEffect(() => {
    ...
    const handleMessage = (msg) => {
      if (msg.type === "transcript" && msg.transcriptType === "final") {
        const newMessage = { content: msg.transcript, role: msg.role };
      ...
      if (
        ...
        if (structuredData && Object.keys(structuredData).length > 0) {
          ...
          console.log("No structured data found when bot said goodbye.");
        }
      }
    };
  ...
  const toggleCall = async () => {
    if (callActive) vapi.stop();
      ...
      try {
        ...
        console.log(import.meta.env.VITE_PUBLIC_VAPI_ASSISTANT_ID);

        await vapi.start(import.meta.env.VITE_PUBLIC_VAPI_ASSISTANT_ID, {
          variableValues: {
            full_name: fullName,
            user_id: user?.id,
          },
  ...
  return (
    <Layout>
      <div className="flex flex-col min-h-screen text-foreground overflow-hidden pb-6 pt-24">
        <div className="container mx-auto px-4 h-full max-w-5xl mb-20">
                ...
                {messages.map((msg, index) => (
                  <div key={index} className="message-item animate-fadeIn">
                    <div className="font-semibold text-xs text-muted-foreground mb-1">
                      {msg.role === "assistant" ? "GenFitPlan AI" : "You"}:
                    </div>
          ...
          <div className="w-full flex justify-center gap-4">
            <Button
              className={`w-40 text-xl rounded-xl ${
                callActive
